 ...
 * standalone at /blokmate/demo. Invoice amount is locale-aware
 * (formatCurrency, TR->TRY / EN,BG->EUR) rather than a hardcoded "80 лв".
 */
const MOCK = {
  building: { name: "Güneşli Sitesi", address: "Vitosha Blvd. 24, Sofya", units: 32 },
  unit: { label: "3B", owner: "A. Petrov" },
  invoice: { amount: 80, dueDate: "2026-09-10", status: "unpaid" as const },
  announcement: {
    ...
    <div className={`card overflow-hidden text-left shadow-soft ${compact ? "max-w-md" : "mx-auto max-w-2xl"}`}>
      ...
      <div className="grid grid-cols-1 gap-4 p-5 sm:grid-cols-2">
        ...
        <div className="rounded-xl border border-line p-4">
          ...
          <div className="mt-1 text-lg font-bold text-ink">{formatCurrency(MOCK.invoice.amount, lang)}</div>
